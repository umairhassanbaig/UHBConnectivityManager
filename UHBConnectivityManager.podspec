#
# Be sure to run `pod lib lint UHBConnectivityManager.podspec' to ensure this is a
# valid spec before submitting.
#
# Any lines starting with a # are optional, but their use is encouraged
# To learn more about a Podspec see http://guides.cocoapods.org/syntax/podspec.html
#

Pod::Spec.new do |s|
  s.name             = 'UHBConnectivityManager'
  s.version          = '0.1.2'
  s.summary          = 'UHBConnectivity manager is an block oriented objective-c wrapper on Reachability. You can observe Internet Connectivity changes by blocks'

# This description is used to generate tags and improve search results.
#   * Think: What does it do? Why did you write it? What is the focus?
#   * Try to keep it short, snappy and to the point.
#   * Write the description between the DESC delimiters below.
#   * Finally, don't worry about the indent, CocoaPods strips it!

  s.description      = <<-DESC
'UHBConnectivity manager is an block oriented objective-c wrapper on Reachability. You can observe Internet Connectivity changes by blocks. You can register any object by giving a unique identifier'
                       DESC

  s.homepage         = 'https://github.com/umairhassanbaig/UHBConnectivityManager'
  # s.screenshots     = 'www.example.com/screenshots_1', 'www.example.com/screenshots_2'
  s.license          = { :type => 'MIT', :file => 'LICENSE' }
  s.author           = { 'Umair Hassan Baig' => 'umairhassanbaig@gmail.com' }
  s.source           = { :git => 'https://github.com/umairhassanbaig/UHBConnectivityManager.git', :tag => s.version.to_s }
  s.social_media_url = 'https://twitter.com/carbonsoftstech'

  s.ios.deployment_target = '7.0'

  s.source_files = 'UHBConnectivityManager/Classes/**/*'
  
  # s.resource_bundles = {
  #   'UHBConnectivityManager' => ['UHBConnectivityManager/Assets/*.png']
  # }

  # s.public_header_files = 'Pod/Classes/**/*.h'
  # s.frameworks = 'UIKit', 'MapKit'
  # s.dependency 'AFNetworking', '~> 2.3'
end
